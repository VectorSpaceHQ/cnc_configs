...
"""%(  os.path.expanduser('~/linuxcnc'),
    ...
    def init_widgets(self):
        ...
        self.configure_jump_button_left_click(self.w.filemanager_usb)
...
#                    QtCore.Qt.Key_F3,QtCore.Qt.Key_F4,QtCore.Qt.Key_F5):
    ...
    def configure_gcodegraphics_middle_pan(self):
        viewer = self.w.gcodegraphics
        if getattr(viewer, '_middle_pan_patched', False):
            return

        original_press = viewer.mousePressEvent
        original_move = viewer.mouseMoveEvent
        original_release = viewer.mouseReleaseEvent

        def middle_pan_press(event):
            buttons = event.buttons()
            if buttons & QtCore.Qt.LeftButton and buttons & QtCore.Qt.MiddleButton:
                viewer._mousemoved = False
                viewer.recordMouse(event.pos().x(), event.pos().y())
                event.accept()
                return
        ...
        def middle_pan_move(event):
            buttons = event.buttons()
            if buttons & QtCore.Qt.LeftButton and buttons & QtCore.Qt.MiddleButton:
                viewer._mousemoved = True
                if not viewer.cancel_rotate:
                    viewer.set_prime(event.pos().x(), event.pos().y())
                    viewer.rotateOrTranslate(event.pos().x(), event.pos().y())
                event.accept()
                return
            if buttons & QtCore.Qt.MiddleButton:
                viewer._mousemoved = True
                viewer.translateOrRotate(event.pos().x(), event.pos().y())
                event.accept()
                return
            original_move(event)

        def middle_pan_release(event):
            if event.button() & QtCore.Qt.MiddleButton:
                event.accept()
                return
            original_release(event)

        viewer.mousePressEvent = middle_pan_press
        viewer.mouseMoveEvent = middle_pan_move
        viewer.mouseReleaseEvent = middle_pan_release
        viewer._middle_pan_patched = True

    def apply_startup_dro_mode(self):
        if self.w.actionbutton_rel.isChecked():
            # Use click events (not setChecked) because DRO action handlers are signal-driven.
            self.w.actionbutton_abs.click()
            self.w.actionbutton_rel.click()
    ...
    def all_homed(self, obj):
        ...
        if self.first_turnon is True:
            ...
            # Retry once after first homing because early startup timing can race DRO updates.
            QtCore.QTimer.singleShot(200, self.apply_startup_dro_mode)
...
<br>'% (property_names.get(i), props[i])
    ...
    def configure_filemanager_path_entry(self, file_manager):
    ...
    def navigate_filemanager_to_typed_path(self, file_manager):
        ...
        if path_line is None:
            return
        raw_path = path_line.text().strip()
        if not raw_path:
            return
        typed_path = os.path.normpath(os.path.expandvars(os.path.expanduser(raw_path)))
        if not os.path.isdir(typed_path):
            self.add_status("Directory not found: {}".format(typed_path), WARNING)
            return
        file_manager.updateDirectoryView(typed_path)

    def configure_jump_button_left_click(self, file_manager):
        jump_button = getattr(file_manager, 'jumpButton', None)
        jump_menu = getattr(file_manager, 'settingMenu', None)
        if jump_button is None:
            return
        if jump_menu is None:
            return
        # Detach the toolbutton menu to prevent long-press expansion.
        jump_button.setMenu(None)
        jump_button.setToolTip('Jump to User directory.\nClick for Options.')
        jump_button.clicked.connect(
            lambda checked=False, btn=jump_button, menu=jump_menu: menu.exec_(btn.mapToGlobal(btn.rect().bottomLeft()))
        )
